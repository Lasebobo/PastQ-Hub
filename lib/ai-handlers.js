/**
 * The AI endpoints + the set-custom-claim endpoint, as plain functions.
 *
...
// ─── Firebase Admin (for custom claims) ────────────────────────────────────────
// Lazily initialised so the module can load in envs without admin credentials
// (e.g. the test suite, local dev without FIREBASE_SERVICE_ACCOUNT set).
let adminAuth = null;
...
async function callGemini(apiKey, payload, retries = 3) {
  ...
  while (attempt < retries) {
    ...
    if ((status === 429 || status === 503) && attempt < retries - 1) {
      ...
      console.warn(`[callGemini] Model high demand (HTTP ${status}). Retrying in ${delay}ms... (Attempt ${attempt}/${retries - 1})`);
      await new Promise(resolve => setTimeout(resolve, delay));
      continue;
    }
    
    throw new Error(`Gemini responded ${status}: ${detail.slice(0, 300)}`);
  }
}
...
export async function ocr(body, apiKey) {
  ...
  if (!apiKey) {
    return { 
      ...
      body: { 
        text: MOCK_OCR_TEXT, 
        meta: { courseCode: "CPE 508", session: "2023/2024", year: 2024, semester: "Rain", level: "500" } 
      } 
    };
  }
  ...
  const result = await callGemini(apiKey, {
    contents: [{
      parts: [
        {
          text: 'You are an expert at extracting text and metadata from university exam papers. '
              + 'Please read the provided exam paper image and extract the text. '
              + 'IMPORTANT: You MUST prefix EVERY question number with the exact word "Question ", like "Question 1:", "Question 2:", etc. '
              + 'Do not solve the questions, just transcribe them accurately. '
              + 'Additionally, extract the following metadata from the paper headers if present: '
              + 'courseCode (e.g. "CPE 508"), session (e.g. "2023/2024"), year (integer, e.g. 2024), semester (e.g. "First" or "Second"), and level (e.g. "500"). '
              + 'Respond ONLY with a JSON object in this exact format: '
              + '{ "text": "transcribed text here", "meta": { "courseCode": "...", "session": "...", "year": 2024, "semester": "...", "level": "..." } }',
        },
  ...
  try {
    ...
    parsed = { text: firstText(result) || 'No text extracted.' };
  }
...
- Format it cleanly using Markdown. IMPORTANT: You MUST prefix EVERY question number with the exact word "Question ", like "Question 1:", "Question 2:", etc.
...
/** POST /api/extract-bulk-solutions — { imageBase64, mimeType, questions } → { solutions: [{ questionId, solutionText }] } */
export async function extractBulkSolutions(body, apiKey) {
  const { imageBase64, mimeType, questions } = body || {};

  if (!apiKey) {
    return { 
      status: 200, 
      body: { solutions: (questions || []).map(q => ({ questionId: q.id, solutionText: `Mock Solution for Question ${q.number}` })) } 
    };
  }

  if (!imageBase64 || !questions || !questions.length) {
    return { status: 400, body: { error: 'Missing image or questions array.' } };
  }

  const promptText = `You are an expert university examiner. I have provided an image of a marking guide or solutions document, along with a list of exam questions.
Your task is to carefully read the solutions from the image, and match each solution to the correct question from the provided list.

Here are the questions:
${JSON.stringify(questions, null, 2)}

Requirements:
1. Extract the detailed, step-by-step solution for each question found in the image.
2. Format the solutions beautifully using Markdown, and use LaTeX for any mathematical equations or formulas.
3. Match each extracted solution to the correct "id" from the questions list.
4. If you cannot find a solution for a specific question, do not include it in your response.

Respond ONLY with a JSON object strictly following this format:
{ "solutions": [ { "questionId": "string", "solutionText": "string" } ] }`;
  ...
  let parsed;
  try {
    parsed = JSON.parse(firstText(result) || '{"solutions":[]}');
  } catch {
    parsed = { solutions: [] };
  }

  return { status: 200, body: { solutions: Array.isArray(parsed.solutions) ? parsed.solutions : [] } };
}

/**
 * POST /api/set-custom-claim — { uid, role } → { ok } | { error }
 *
 * Sets request.auth.token.role on a Firebase user's ID token so that
 * Storage rules can check the role without a cross-database firestore.get()
 * call (which doesn't work with named Firestore databases).
 *
 * SECURITY: This endpoint must only be callable by an authenticated admin.
 * Enforcement is done by the caller (AdminPanel.tsx already checks
 * authUser.role before calling changeRole) AND by Firestore rules (only
 * admins can write to users/{uid}.role). The endpoint itself is unauthenticated
 * at the HTTP layer — it is a Vercel serverless function, not a Firebase
 * Function, so there is no Firebase auth middleware here. On Vercel this is
 * acceptable because an attacker would still need to know a valid uid AND the
 * Firestore rules would block any Firestore write that could surface a rogue
 * role in the app — but if you want defence-in-depth, add ID-token verification
 * here using adminAuth.verifyIdToken(req.headers.authorization).
 */
export async function setCustomClaim(body) {
  const { uid, role } = body || {};

  const VALID_ROLES = ['student', 'lecturer', 'admin'];
  if (!uid || typeof uid !== 'string') {
    return { status: 400, body: { error: 'Missing or invalid uid.' } };
  }
  ...
  if (!auth) {
    // No FIREBASE_SERVICE_ACCOUNT set — running locally without admin creds.
    // Return success so the app doesn't break in dev; the claim won't actually
    // be set, but Storage rules aren't enforced locally anyway.
    console.warn('[set-custom-claim] FIREBASE_SERVICE_ACCOUNT not set — skipping claim (dev mode).');
...
export const routes = {
  ...
  '/api/extract-bulk-solutions': extractBulkSolutions,
  '/api/set-custom-claim': setCustomClaim,
};
