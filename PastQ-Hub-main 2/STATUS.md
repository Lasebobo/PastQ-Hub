...
**CPE 508 · Obafemi Awolowo University** · 8 September 2026 · branch `main`
...
| Discussion forum | moderation was decorative | real-time threads, subcollection comments, view counts and real flags |
...
| Security rules | **48 / 48** | Every collection, every role, against a real Firestore emulator. |
...
returned it to a clean pass. **The suite detects the thing it guards.**

```bash
npm run test:rules   # boots the emulator, runs 48 assertions, tears down
```
...
- **Forum attachments are deliberately absent.** Firebase Storage is not configured, so
  the old paperclip/image controls were removed rather than leaving a dead affordance.
